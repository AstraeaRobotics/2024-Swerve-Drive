...
import frc.robot.util.AngleMapping;
...
public class TeleopSwerve extends CommandBase {
  ...
  public TeleopSwerve(SwerveSubsystem swerveSubsystem, DoubleSupplier driveX, DoubleSupplier driveY, DoubleSupplier rotationX, DoubleSupplier rotationY) {
  ...
  public void execute() {
    ...
    if (Math.abs(this.m_rotationX.getAsDouble()) >= .1 || Math.abs(this.m_rotationY.getAsDouble()) >= .1) 
      rotationAngle = -AngleMapping.getAngle(this.m_rotationX.getAsDouble(), this.m_rotationY.getAsDouble(), m_swerveSubsystem.getHeading()) + 180;
