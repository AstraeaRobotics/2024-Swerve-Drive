...
public class SwerveModule extends SubsystemBase {
  ...
  public PIDController turnPIDController = new PIDController(0.004, 0, 0.0001);

  public SlewRateLimiter driveSlewRateLimiter = new SlewRateLimiter(0.7);
  ...
  public SwerveModule(String name, int angleCanId, int driveCanId, double angleDisplacement) {
    ...
    // this.driveEncoder.setPositionConversionFactor((DriveConstants.kWheelDiameter * Math.PI) / DriveConstants.kDriveMotorGearRatio);
  ...
  public void move(double driveSpeed, double turnSpeed) {
    ...
    this.turnMotor.set(MathUtil.clamp(turnSpeed, -DriveConstants.kMaxTurnSpeed, DriveConstants.kMaxTurnSpeed));
  ...
  public double getTurnMotorSpeed(double displacement) {
    return turnPIDController.calculate(displacement, 0);
  }
